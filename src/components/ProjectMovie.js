...
const Projetcs = () => {
    ...
    return (
        <div className='flex flex-col w-full'>
            <div id='home' className={`flex flex-col justify-evenly items-center h-[20rem] w-full p-2 ${screenize.width > 1000 ? 'mt-[2%]' : 'mt-[5%]'}`}>
                <p className={`text-center font-bold flex items-end ${screenize.width > 1000 ? 'text-6xl' : 'text-3xl'}`}><RocketLaunchIcon className='h-[5rem] w-[5rem] text-orange-400'/><p className='text-white'>Rocket-bank</p></p>
                <p className={`${screenize.width > 1000 ? 'w-[40%] text-lg' : 'w-[90%] text-md'} text-center text-white`}>Rocket Bank is an online banking platform designed to enable administrators to manage user accounts effectively.</p>
                <button className={`bg-none border-2 border-white p-2 h-[4rem] ${screenize.width > 1000 ? 'w-[18%] text-lg' : 'w-[40%] text-sm'}  rounded text-xl font-bold text-white shadow-md uppercase transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 hover:bg-gray-950/40 duration-300`}><a href='https://github.com/Gilson96/React_E-bank/'>github </a></button>
            </div>

            <div className={`flex flex-col ${screenize.width > 1000 ? '' : ''} justify-center items-center px-10 pt-10`}>
                <div className='w-[50%] flex items-center justify-center '>
                    <div className={`flex justify-center items-center ${screenize.width > 1000 ? 'h-[25rem] w-[35rem]' : 'h-[10rem] w-[17rem] p-5'}  border border-slate-100 rounded-3xl bg-sky-950 mb-6`}>
                        <p><RocketLaunchIcon className={`${screenize.width > 1000 ? 'h-24 w-24' : 'h-10 w-10'} text-[#F2AF29]`} /></p>
                        <p id='logoFont' className='text-4xl italic'>Rocket<span className='text-xl'>Bank</span></p>
                    </div>
                </div>
                <div className={`flex flex-col gap-5 text-white ${screenize.width > 1000 ? 'w-[50%]' : 'mt-[3%] w-full'} `}>
                    ...
                    <div className='flex gap-2 mb-[6%]'>
                        <button className={`bg-none border-2 border-white py-3 px-5 ${screenize.width > 1000 && 'h-[3rem] w-auto'}  rounded text-md w-auto font-bold text-white shadow-lg uppercase mt-3 transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 hover:bg-gray-950/40 duration-300`}><a href='https://gilson96.github.io/rocket-bank/'>see live</a></button>
                    </div>
                </div>
            </div>
        </div>
